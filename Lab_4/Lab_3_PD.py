...
def made_annotations (dataset_path,folder_path):
    # 1_part
    with open(f'{folder_path}/xset.csv', 'w') as xdataset, open(f'{folder_path}/yset.csv', 'w') as ydataset:
    ...
    for i in range(len(boundaries) - 1):
        ...
        filenames.append(folder_path +"/" + filename + ".csv")
...
def find_dataset(file_path, date_input_entry):
    
    # Получаем дату от пользователя.
    # date = datetime.datetime.strptime(input("Введите дату: "), "%Y-%m-%d")
    date = datetime.datetime.strptime(date_input_entry, "%Y-%m-%d")
    return get_data_from_dataset(date, file_path)
    ...
    #     results = get_data_from_dataset(date, file_path)
    #     if results is None:
    #         print("Нет результатов поиска.")
...
def create_dataset(dataset_path, folder_path):
    made_annotations(dataset_path,folder_path)
